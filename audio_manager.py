...
class AudioManager:
    ...
    def _preload_sounds(self):
        """Preload SFX to eliminate latency on button taps."""
    ...
    def play_bgm(self, track_name: str, force_restart: bool = False):
        ...
        if self._current_bgm_track == track_name and self._current_bgm_sound:
            if self._current_bgm_sound.state == "play":
                return
        ...
        try:
            ...
            print(f"[AudioManager] Error playing BGM '{track_name}': {err}")
    ...
    def stop_bgm(self):
        ...
        if self._current_bgm_sound:
            try:
                ...
                self._current_bgm_sound.unload()
            except Exception:
                pass
    ...
    def pause_bgm(self):
        ...
        if self._current_bgm_sound and self._current_bgm_sound.state == "play":
            try:
                self._current_bgm_sound.stop()
            except Exception:
                pass
    ...
    def play_sfx(self, sfx_name: str):
        ...
        if sound:
            try:
                ...
                # Stop if currently playing so rapid taps trigger immediate sound
                if sound.state == "play":
                    sound.stop()
                sound.play()
            except Exception as e:
                print(f"[AudioManager] Error playing SFX '{sfx_name}': {e}")
